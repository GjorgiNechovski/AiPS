...
public class belaTehnika {
    public static void main(String[] args) throws IOException {
        ...
        Heap<Customer> customers = new Heap<>(n);

        bf.lines().forEach(x->customers.insert(new Customer(x)));

        int time = 0,count=0,max=0;

        for (int i=0; i<customers.getSize(); i++){
            Customer temp = customers.removeMax();

            count=1;
            for (int j=0; j<customers.getSize(); j++){
                if(temp.sameTime(customers.getAt(j)))
                    count++;
            }
            if(count>max)
                max=count;
...
class Customer implements Comparable<Customer> {
    ...
    public Customer(String x) {
       ...
       this.entry = Integer.parseInt(time[0]) * 60 + Integer.parseInt(time[1]);
       this.duration = Integer.parseInt(parts[1]);

       if(entry>23*60+59)
           duration = entry + duration - 23*60+59;
    }

    public boolean sameTime(Customer c){
        return entry+duration>=c.entry;
    }

    @Override
    public int compareTo(Customer o) {
        return  Integer.compare(o.entry + o.duration, entry+duration);
    }
}
