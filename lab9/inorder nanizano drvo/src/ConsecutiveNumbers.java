...
class BTree<E> {
    ...
    public BNode<E> findElement (BNode<E> temp){
        if (temp.rtag == '-')
            return temp.right;

        BNode<E> node = temp.right;
        while (node.ltag=='+')
            node = node.left;
...
public class ConsecutiveNumbers {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(najdi(tree));

    }

    public static boolean najdi(BTree<Integer> tree){
        BNode<Integer> go = tree.head.left;

        while(go.ltag!='-')
            go = go.left;

        while (go!=tree.head){
            if(tree.findElement(go).info!=null && tree.findElement(go).info - go.info !=1)
                return false;

            go = tree.findElement(go);
