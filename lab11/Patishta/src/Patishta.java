...
class Graph<E> {
    ...
    int getIndex(E value){
        ...
        Arrays.stream(adjList).filter(x->x.getInfo().equals(value))
    }
...
public class Patishta{
    public static void main(String[] args) throws IOException {
        BufferedReader bf = new BufferedReader(new InputStreamReader(System.in));

        int numOfCities = Integer.parseInt(bf.readLine());
        int numOfRoads = Integer.parseInt(bf.readLine());

        Graph<String> graph = new Graph<>(numOfCities);

        for (int i=0; i<numOfRoads; i++){
            String[] parts = bf.readLine().split("\\s+");

            graph.adjList[Integer.parseInt(parts[0])].setInfo(parts[1]);
            graph.adjList[Integer.parseInt(parts[2])].setInfo(parts[3]);

            graph.addEdge(Integer.parseInt(parts[0]),Integer.parseInt(parts[2]),Float.parseFloat(parts[4]));
        }
