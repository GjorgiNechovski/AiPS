...
public class Heroes {
    ...
    public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards) {
        SLLNode<Card> best = firstFriendCards.getFirst();
        SLLNode<Card> temp = firstFriendCards.getFirst();
        while(temp!=null){
            if (temp.element.damage()>best.element.damage())
                best=temp;
            temp=temp.succ;
        }

        Card o = new Card(best.element.getId(), best.element.getPower(), best.element.getNumAttacks());

        temp = secondFriendCards.getFirst();
        for (int i=0; i<2; i++)
            temp=temp.succ;

        secondFriendCards.insertAfter(o,temp);
        firstFriendCards.delete(best);
    }
