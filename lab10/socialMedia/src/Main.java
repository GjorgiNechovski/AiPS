...
public class Main {
    public static void main(String[] args) {
        ...
        int n = in.nextInt(); //number of users
        ...
        for (int i=0; i<n; i++){
            int sizeForJ = in.nextInt();

            for (int j=0; j<sizeForJ; j++){
        ...
        int indexOfUser1 = in.nextInt(), indexOfUser2 = in.nextInt();

        Queue<Profile> queue = new LinkedList<>();
        queue.add(profiles[indexOfUser1]);

        while (!queue.isEmpty()){
            Profile temp = queue.poll();

            if(temp.index==indexOfUser2){
                System.out.println(temp.distance);
                break;
            }

            for (int i=0; i<n; i++){
                if(graph.adjacent(temp.index,i)==1 && !visted[i]){
                    queue.add(profiles[i]);
                    visted[i] = true;
                    profiles[i].distance = temp.distance+1;
                }
            }
        }
    }
